...
public class ModUnits extends Units {
    ...
    public static void unitDeath(int uid) {
        ...
        for (Entityc entityc : Groups.all) {
            if (entityc instanceof Unit && entityc.id() == uid) {
                unit = entityc.as();
                break;
            }
        }
    ...
    public static boolean invalidateTarget(Posc target, Team team, float x, float y, float range) {
        ...
        Healthc h;
        return target == null ||
               (range != Float.MAX_VALUE && !target.within(x, y, range + (target instanceof Sized && (hb = (Sized) target) == target ? hb.hitSize() / 2f : 0f))) ||
               (target instanceof Teamc && (t= (Teamc) target)==target && t.team() == team) ||
               (target instanceof Healthc && (h= (Healthc) target)==target && !h.isValid());
    }
    ...
    Building closestBuilding(Team team, float wx, float wy, float range, Boolf<Building> pred) {
        buildResult = null;
        cdist = 0f;

        QuadTree<Building> buildings = team.data().buildings;
    ...
    public static Unit bestEnemy(Team team, float x, float y, float range, Boolf<Unit> predicate, Units.Sortf sort) {
        ...
        nearbyEnemies(team, x - range, y - range, range * 2f, range * 2f, e -> {
            if (e.dead() || !predicate.get(e) || e.team == Team.derelict || !e.within(x, y, range + e.hitSize / 2f))
                return;
     ...
     * Returns the closest ally of this team in a range. Filter by predicate.
     */
    public static Unit closest(Team team, float x, float y, float range, Boolf<Unit> predicate, Units.Sortf sort) {
        result = null;
        cdist = 0f;
    ...
    public static int count(float x, float y, float width, float height, Boolf<Unit> filter) {
        ...
        for (Entityc entityc : Groups.all) {
            if (entityc instanceof Unit) {
                ...
                unit.hitbox(Tmp.r1);
                if (Tmp.r1.overlaps(x, y, width, height) && filter.get(unit)) intResult++;
            }
        }
        return intResult;
    }
    ...
    public static void nearby(@Nullable Team team, float x, float y, float width, float height, Cons<Unit> cons) {
        for (Entityc entityc : Groups.all) {
            if (entityc instanceof Unit) {
                Unit unit = entityc.as();
                unit.hitbox(Tmp.r1);
                if (Tmp.r1.overlaps(x, y, width, height) && (team != null && unit.team == team || state.teams.getActive().contains(d -> d.team == unit.team))) {
                    cons.get(unit);
                }
            }
        }
    ...
    public static void nearby(float x, float y, float width, float height, Cons<Unit> cons) {
        for (Entityc entityc : Groups.all) {
            if (entityc instanceof Unit) {
                Unit unit = entityc.as();
                unit.hitbox(Tmp.r1);
                if (Tmp.r1.overlaps(x, y, width, height)) {
                    cons.get(unit);
                }
            }
        }
