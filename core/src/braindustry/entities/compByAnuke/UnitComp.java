...
abstract class UnitComp implements Healthc, Physicsc, Hitboxc, Statusc, Teamc, Itemsc, Rotc, Unitc, Weaponsc, Drawc, Boundedc, Syncc, Shieldc, Commanderc, Displayable, Senseable, Ranged, Minerc, Builderc{
    ...
    /** @braindustry.annotations.ModAnnotations.return approx. square size of the physical hitbox for physics */
    public float physicSize(){
        return hitSize * 0.7f;
    }

    /** @braindustry.annotations.ModAnnotations.return whether there is solid, un-occupied ground under this unit. */
    ...
    /** @braindustry.annotations.ModAnnotations.return speed with boost multipliers factored in. */
    ...
    /** @braindustry.annotations.ModAnnotations.return where the unit wants to look at. */
...
return controller instanceof FormationAI && isValid() ? 1 : 0;
            case payloadCount:
Payloadc pay;
return ((Object)this) instanceof Payloadc && (pay  = (Payloadc)((Object)this))==((Object)this)
? pay.payloads().size : 0;
...
return Float.NaN;
    ...
    public Object senseObject(LAccess sensor){
        switch(sensor){            case type:
return type;
            case name:
Player p;
return controller instanceof Player && (p  = (Player)controller)==controller
? p.name : null;
...
LogicAI log;
FormationAI form;
return !isValid() ? null : controller instanceof LogicAI && (log  = (LogicAI) controller)== controller
? log.controller : controller instanceof FormationAI && (form  = (FormationAI) controller)== controller
? form.leader : this;
...
BuildPayload p2;
return ((Object)this) instanceof Payloadc && (pay  = (Payloadc)((Object)this))==((Object)this)
?                (pay.payloads().isEmpty() ? null :                pay.payloads().peek() instanceof UnitPayload && (p1  = (UnitPayload)                pay.payloads().peek())==                pay.payloads().peek()
? p1.unit.type :                pay.payloads().peek() instanceof BuildPayload && (p2  = (BuildPayload)                pay.payloads().peek())==                pay.payloads().peek()
? p2.block() : null) : null;
...
return noSensed;
    ...
    /** @braindustry.annotations.ModAnnotations.return pathfinder path type for calculating costs */
    public int pathType(){
    ...
    /** @braindustry.annotations.ModAnnotations.return a preview icon for this unit. */
    ...
    /** @braindustry.annotations.ModAnnotations.return name of direct or indirect player controller. */
    @Override
    public @Nullable String getControllerName(){
        if(isPlayer()) return getPlayer().name;
LogicAI ai;
        if(controller instanceof LogicAI && (ai  = (LogicAI)controller)==controller
&& ai.controller != null) return ai.controller.lastAccessed;

FormationAI aif;
        if(controller instanceof FormationAI && (aif  = (FormationAI)controller)==controller
&& aif.leader != null && aif.leader.isPlayer()) return aif.leader.getPlayer().name;
