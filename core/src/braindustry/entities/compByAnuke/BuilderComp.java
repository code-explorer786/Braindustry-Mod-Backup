...
abstract class BuilderComp implements Posc, Statusc, Teamc, Rotc{
    ...
    @braindustry.annotations.ModAnnotations.Import float x, y, rotation, buildSpeedMultiplier;
    @braindustry.annotations.ModAnnotations.Import UnitType type;
    @braindustry.annotations.ModAnnotations.Import Team team;

    @braindustry.annotations.ModAnnotations.SyncLocal Queue<BuildPlan> plans = new Queue<>(1);
    @braindustry.annotations.ModAnnotations.SyncLocal boolean updateBuilding = true;
...
ConstructBuild entity;
        ...
        //otherwise, update it.
        if(current.breaking){
            entity.deconstruct(self(), core.as(), 1f / entity.buildCost * Time.delta * type.buildSpeed * buildSpeedMultiplier * state.rules.buildSpeedMultiplier);
        }else{
    ...
    /** @braindustry.annotations.ModAnnotations.return whether this request should be skipped, in favor of the next one. */
